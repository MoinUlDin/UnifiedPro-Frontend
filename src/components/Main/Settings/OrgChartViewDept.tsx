...
import type { DeptNode } from '../../../constantTypes/CompanySetupTypes';
...
const OrgChartViewDept: React.FC<Props> = ({ data = [], search = '', filterDepartment = null, onEdit, onDelete, className = '' }) => {
    ...
    const { idToNode, parentMap } = useMemo(() => {
        ...
        const walk = (nodes?: DeptNode[], parentId?: string) => {
            if (!nodes) return;
            nodes.forEach((n) => {
    ...
    const visibleNodes = useMemo(() => {
        const list: { node: DeptNode; parentId?: string }[] = [];
        const walk = (nodes?: DeptNode[], parentId?: string) => {
            if (!nodes) return;
            nodes.forEach((n) => {
    ...
    const drawConnectors = useMemo(() => {
        return () => {
            ...
            const drawForNodes = (nodes?: DeptNode[]) => {
                if (!nodes) return;
                nodes.forEach((n) => {
    ...
    // node card
    const NodeCard: React.FC<{ node: DeptNode }> = ({ node }) => {
                ...
                <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                            ...
                            <div className="text-[11px] text-gray-400">{node.employees_count ?? 0} employees</div>
                        </div>
                    </div>
                </div>
            </div>
...
export default OrgChartViewDept;
