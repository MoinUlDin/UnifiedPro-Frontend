...
export default function DeductionTable() {
    ...
    const handleDelete = (id: number) => {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'Do you really want to delete this Deduction? This action is irreversible!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                SalaryServices.DeleteDeduction(id)
                    .then(() => {
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-2 items-center justify-center sm:justify-between">
                <div>
                    ...
                    <p className="text-gray-600">Manage tax and deduction percentages</p>
                </div>
                ...
                <table className="min-w-full text-left">
                    <thead className="">
                        <tr className="border-b ">
                            <th className="py-3 px-4 text-sm font-semibold bg-white text-gray-800">Deduction Name</th>
                            <th className="py-3 px-4 text-sm font-semibold bg-white text-gray-800">Percentage</th>
                            <th className="py-3 px-4 text-sm font-semibold bg-white text-gray-800">Type</th>
                    ...
                    <tbody className="divide-y">
                        {Deduction.map((job) => (
                            <tr key={job.id}>
                                ...
                                <td className="py-3 px-4 ">{job.percentage}</td>
                                <td className="py-3 px-4">
                                    <span className="px-2 py-1 text-gray-800 text-[12px] rounded-lg shadow">{job.type}</span>
                                </td>
                                <td className="py-3 px-4">
            ...
            {openDeduction && <DeductionPopup initialData={init} show={openDeduction} onClose={() => setOpenDeduction(false)} sendResponse={hanldeResponse} />}
        </div>
    );
}
