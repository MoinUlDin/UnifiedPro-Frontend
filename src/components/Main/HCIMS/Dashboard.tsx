...
const Index = () => {
    ...
    const isRtl = useSelector((state: RootState) => state.themeConfig.rtlClass === 'rtl'); // Correctly define isRtl
    ...
    return (
        <div ref={dashboardRef}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="flex justify-between text-[#236192] mb-5">
                        ...
                        <div className="dropdown">
                            <Dropdown
                                ...
                                button={<IconHorizontalDots className="text-[#236192] hover:!text-[#772583]" />}
                            >
                                <ul>
                                    <li>
                ...
                >
                    <div className="flex items-start justify-between text-[#ffffff] mb-16 z-[7]">
                        ...
                        <div className="relative text-xl whitespace-nowrap">85%</div>
                    </div>
                    <div className="flex items-center justify-between z-10">
                        ...
                        <button type="button" className="shadow-[0_0_2px_0_#ffffff] rounded p-1 text-[#ffffff] hover:bg-[#772583] z-10">
                            View Details
                        </button>
                    </div>
                </div>
            ...
            <ResponsiveGridLayout className="layout" layouts={layouts} breakpoints={breakpoints} cols={cols} rowHeight={30} onLayoutChange={onLayoutChange} isDraggable={true} isResizable={true}>
            ...
            <Modal isOpen={isModalOpen} onRequestClose={closeModal} contentLabel="Add Widget" style={{ content: { width: '50%', margin: 'auto', height: 'auto' } }}>
                ...
                <input type="text" value={widgetTitle} onChange={(e) => setWidgetTitle(e.target.value)} className="p-2 border rounded-lg w-full mb-4" />
                <label className="font-semibold">Widget Description:</label>
                <textarea value={widgetDescription} onChange={(e) => setWidgetDescription(e.target.value)} className="p-2 border rounded-lg w-full mb-4" />
                <label className="font-semibold">Widget Type:</label>
                <Select options={widgetTypes} value={selectedWidgetType} onChange={setSelectedWidgetType} className="mb-4" />

                {/* Conditional UI based on selected widget type */}
                {selectedWidgetType.value === 'chart' && (
                    <>
                        <label className="font-semibold">Chart Type:</label>
                        <Select options={chartTypes} value={chartType} onChange={setChartType} className="mb-4" />
                    </>
                )}
                    ...
                    <>
                        ...
                        {tableData.map((row, index) => (
                            <div key={index} className="flex space-x-4 mb-4">
                                <input type="text" placeholder="Name" value={row.name} onChange={(e) => handleTableRowChange(index, 'name', e.target.value)} className="p-2 border rounded-lg w-full" />
                                <input
                                    type="text"
                                    placeholder="Value"
                                    value={row.value}
                        ...
                        <button onClick={addTableRow} className="bg-gray-500 text-white px-4 py-2 rounded-lg">
                            Add Row
                        </button>
                    </>
                )}
                ...
                {selectedWidgetType.value === 'summary' && (
                    <>
                        ...
                        <textarea value={summaryText} onChange={(e) => setSummaryText(e.target.value)} className="p-2 border rounded-lg w-full mb-4" />
                    </>
                )}

                <label className="font-semibold">Widget Width (1-12):</label>
                <input type="number" value={modalWidth} onChange={(e) => setModalWidth(Number(e.target.value))} className="p-2 border rounded-lg w-full mb-4" />
                <label className="font-semibold">Widget Height (in rows):</label>
                <input type="number" value={modalHeight} onChange={(e) => setModalHeight(Number(e.target.value))} className="p-2 border rounded-lg w-full mb-4" />
                <button onClick={addCustomContainer} className="bg-green-500 text-white px-4 py-2 rounded-lg">
                    Add Widget
                </button>
                <button onClick={closeModal} className="bg-red-500 text-white px-4 py-2 rounded-lg ml-2">
                    Cancel
                </button>
            </Modal>
        </div>
