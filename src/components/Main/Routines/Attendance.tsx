    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => navigate("/view-marked-holidays")}
          ...
          <div className="flex items-center bg-white dark:bg-[#1c232f] rounded-lg shadow-sm p-1">
            <button
              ...
              className={`p-2 ${view === 'list' ? 'bg-primary/10 text-primary' : 'text-gray-500'} rounded transition-colors`}
            >
              <IconUser className="w-5 h-5" />
            </button>
            <button
              onClick={() => setView('card')}
              className={`p-2 ${view === 'card' ? 'bg-primary/10 text-primary' : 'text-gray-500'} rounded transition-colors`}
            >
              <IconCalendar className="w-5 h-5" />
            </button>
          </div>
        ...
        {[
          ...
          <div key={stat.label} className="bg-white dark:bg-[#1c232f] rounded-lg shadow-sm p-4">
            ...
            <div className="text-gray-400 text-sm mt-1">
              {((stat.value / getAttendanceStats().total) * 100).toFixed(1)}%
            </div>
          </div>
        ))}
      </div>
              ...
              <tbody>
                {attendance.map((emp) => (
                  <tr key={emp.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="p-4">
                      {emp.records[0] && (
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(emp.records[0].status)}`}>
                          {emp.records[0].status}
                        </span>
                      )}
                    </td>
                ...
                {emp.records.map((record, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 rounded-lg border dark:border-gray-700">
                    <div>
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${getStatusColor(record.status)}`}>
                        {record.status}
                      </span>
                    </div>
                    {record.checkIn && (
